...
public class ClasseurParMots {
	public void execute() throws Exception {

		JFrame jf = new JFrame("Classeur les données par mots");
		jf.setSize(400, 450);
		...
		openBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				showClasseurMotsDialog(jf,msgTextArea);
			}
		});
		panel.add(openBtn);
	 ...
	 * ouvrir fichier
	 */
	private static void showClasseurMotsDialog(Component parent, JTextArea msgTextArea) {
				...
				for (int i = 0; i < files.size(); i++) {
					...
					for (int j = i; j < traces.size(); j++) {
						if (traces.get(j).getExpectedValue()!=null) {
							File newFile = new File(save.getPath(), traces.get(j).getExpectedValue());
							if(!newFile.exists()) {
								newFile.mkdir();
							}
